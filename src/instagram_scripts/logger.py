...
instagram_handler_error = logging.FileHandler(Path(__file__).parent / "logs" / "error.log")
instagram_handler_error.setLevel(logging.ERROR)
instagram_handler_error.setFormatter(formatter)

instagram_handler_critical = logging.FileHandler(Path(__file__).parent / "logs" / "critical.log")
